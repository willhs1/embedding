        ...
        with  open("embeddings_data/"+str_dataset,"r") as f_e:
            try:
                for i,line in enumerate(f_e):
                    ...
                    #obtem o embedding
                    dict_embedding[word] = np.array(arr_line[len(arr_line)-embedding_size:], dtype=np.float16)
                    if(i%10000 == 0):
                        print(f"{i}: {word}")
            except ValueError:
...
class KDTreeEmbedding:
    ...
    def get_most_similar_embedding(self,query:Union[np.array,str],k_most_similar:int=5,words_to_ignore:List=[]):
        ...
        #obtém o embedding
        nearest_dist, nearest_ind = self.kd_embedding.query([query_embedding], k=k_most_similar, return_distance=True)
    ...
    def get_embeddings_by_similarity(self,query:np.array, max_distance:float, words_to_ignore:List=[]):
        ...
        nearest_ind, nearest_dist =  self.kd_embedding.query_radius([embedding], r=max_distance, return_distance=True)  
